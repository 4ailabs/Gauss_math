...
const StyledTextarea: React.FC<React.TextareaHTMLAttributes<HTMLTextAreaElement>> = (props) => (
  <textarea
    {...props}
    className="w-full h-full p-4 bg-slate-900/50 border border-slate-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none resize-none transition-colors"
  />
);
...
const TabButton: React.FC<{onClick: () => void; active: boolean; children: React.ReactNode; icon: React.ReactNode}> = ({ onClick, active, children, icon }) => (
    <button
      onClick={onClick}
      className={`flex-1 flex flex-col items-center gap-1 py-3 px-1 text-sm font-medium transition-colors ${
...
`;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 p-4 sm:p-6 lg:p-8">
      <div className="max-w-screen-2xl mx-auto flex flex-col min-h-[calc(100vh-2rem)]">
        ...
        <div className="lg:hidden mb-4 border-b border-slate-700">
            <div className="flex justify-around">
                <TabButton onClick={() => setActiveView('editor')} active={activeView === 'editor'} icon={<BookOpenIcon className="w-5 h-5"/>}>Editor</TabButton>
                <TabButton onClick={() => setActiveView('assistant')} active={activeView === 'assistant'} icon={<MessageCircleIcon className="w-5 h-5"/>}>IA</TabButton>
                <TabButton onClick={() => setActiveView('summary')} active={activeView === 'summary'} icon={<BrainCircuitIcon className="w-5 h-5"/>}>Resultados</TabButton>
            </div>
        </div>

        <main className="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-grow">
          {/* Column 1: Editor */}
          <div className={`flex-col gap-4 lg:col-span-1 ${activeView === 'editor' ? 'flex' : 'hidden'} lg:flex`}>
            <h2 className="text-2xl font-semibold text-white flex items-center gap-2">
             ...
             <div className="relative">
                ...
                <select
                    ...
                    className="w-full bg-slate-900/50 border border-slate-700 rounded-lg p-3 pl-10 text-white focus:ring-2 focus:ring-blue-500 focus:outline-none appearance-none"
                >
                    {subjects.map(subject => ( <option key={subject} value={subject}>{subject}</option>))}
                </select>
             </div>

            <div className="bg-slate-900 rounded-lg p-1 shadow-lg flex-grow flex flex-col min-h-[550px]">
                <StyledTextarea
            ...
            <div className="flex flex-col sm:flex-row gap-4">
                 <button onClick={handleProcessNotes} disabled={isLoading || isScanning || isExporting || isRecording} className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-700 text-white font-bold py-3 px-4 rounded-lg transition-all transform hover:scale-105">
                    {isLoading ? <><LoaderCircleIcon className="animate-spin w-5 h-5"/> Procesando...</> : <><BrainCircuitIcon className="w-5 h-5"/> Procesar Apuntes</>}
                </button>
                <button
                    onClick={handleToggleRecording}
                    disabled={isLoading || isScanning || isExporting || !isSpeechSupported}
                    className={`w-full sm:w-auto flex items-center justify-center gap-2 font-bold py-3 px-4 rounded-lg transition-all ${
                        !isSpeechSupported
                ...
                >
                    {isRecording ? (
                        <>
                            <MicIcon className="w-5 h-5 animate-pulse" /> Detener
                        </>
                    ) : (
                        <>
                            <MicIcon className="w-5 h-5" /> Grabar
                        </>
                    )}
                </button>
                <button 
          ...
          <div className={`flex-col gap-4 lg:col-span-1 ${activeView === 'assistant' ? 'flex' : 'hidden'} lg:flex`}>
            <div className="flex justify-between items-center">
             ...
             <div className="bg-slate-900/70 rounded-lg shadow-lg flex-grow flex flex-col p-4 min-h-[660px]">
                <AssistantView 
          ...
          <div className={`flex-col gap-4 lg:col-span-1 ${activeView === 'summary' ? 'flex' : 'hidden'} lg:flex`}>
            <h2 className="text-2xl font-semibold text-white flex items-center gap-2">
                <BrainCircuitIcon className="w-6 h-6 text-blue-400"/>
                Apuntes Procesados
            </h2>
            <div id="processed-output" className="bg-slate-900/70 rounded-lg shadow-lg flex-grow p-6 min-h-[660px]">
              <SummaryView data={processedData} isLoading={isLoading} onExport={handleExportToPdf} isExporting={isExporting} />
            </div>
          </div>
        </main>

        <footer className="text-center mt-8 py-4 border-t border-slate-800">
            <p className="text-slate-500 text-sm">Desarrollado por 4ailabs</p>
        </footer>
      </div>
    </div>
  );
...
const Flashcard: React.FC<{ concept: string; definition: string }> = ({ concept, definition }) => {
    ...
    return (
        <div className="[perspective:1000px] w-full" onClick={() => setIsFlipped(!isFlipped)} role="button" tabIndex={0} onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && setIsFlipped(!isFlipped)}>
            <div className={`relative w-full h-32 rounded-lg shadow-md cursor-pointer transition-transform duration-700 [transform-style:preserve-3d] ${isFlipped ? '[transform:rotateY(180deg)]' : ''}`}>
                <div className="absolute inset-0 w-full h-full flex flex-col items-center justify-center bg-slate-800/70 p-4 rounded-lg [backface-visibility:hidden] border border-slate-700 text-center">
                    <h4 className="text-md font-bold text-white">{concept}</h4>
                    <div className="absolute bottom-2 right-2 flex items-center gap-1 text-xs text-slate-400">
                        <RefreshCwIcon className="w-3 h-3"/> Voltear
                    </div>
                </div>
                <div className="absolute inset-0 w-full h-full bg-slate-800 p-4 rounded-lg [backface-visibility:hidden] [transform:rotateY(180deg)] overflow-y-auto">
...
const SummaryView: React.FC<{data: ProcessedData | null, isLoading: boolean, onExport: () => void, isExporting: boolean}> = ({ data, isLoading, onExport, isExporting }) => {
    if (isLoading) return <div className="flex items-center justify-center h-full"><LoaderCircleIcon className="w-10 h-10 animate-spin text-blue-400" /></div>;
    if (!data) return <div className="flex items-center justify-center h-full text-slate-400 text-center p-4"><p>Procesa tus apuntes para ver el análisis de la IA aquí.</p></div>;
        ...
        <div className="animate-fade-in h-full flex flex-col">
            <div className="flex-shrink-0 mb-4 flex justify-end">
                <button 
                    ...
                    className="flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 disabled:bg-slate-700 disabled:opacity-50 text-white font-bold py-2 px-3 rounded-lg transition-all"
                >
                    {isExporting ? 
                        <><LoaderCircleIcon className="animate-spin w-5 h-5"/> Exportando...</> : 
                        <><DownloadIcon className="w-5 h-5"/> Exportar PDF</>
                    }
                </button>
            </div>
            <div id="processed-output-content" className="space-y-8 overflow-y-auto flex-grow pr-2">
                <div>
                    <h3 className="text-xl font-bold text-blue-400 mb-2">Resumen</h3>
                    <p className="text-slate-300 whitespace-pre-wrap">{data.summary}</p>
                </div>
                <div>
                    <h3 className="text-xl font-bold text-blue-400 mb-4">Conceptos Clave (Flashcards)</h3>
                 ...
                 {data.relatedProblems?.length > 0 && (
                     <div>
                        <h3 className="text-xl font-bold text-blue-400 mb-3">Problemas para Practicar</h3>
                        <ul className="space-y-3">
                            {data.relatedProblems.map((item, index) => (
                               <details key={index} className="p-3 bg-slate-900/50 rounded-md cursor-pointer group">
                                   <summary className="font-semibold text-white list-none flex justify-between items-center gap-2">
                                       <span className="flex-1">{item.problem}</span>
                                       <svg className="w-4 h-4 transition-transform transform group-open:rotate-90" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" /></svg>
                                   </summary>
                                   <div className="text-slate-300 mt-2 pt-2 border-t border-slate-700 whitespace-pre-wrap"><strong>Solución:</strong> {item.solution}</div>
                               </details>
                            ))}
                        </ul>
                    </div>
                 ...
                 <div>
                    <h3 className="text-xl font-bold text-blue-400 mb-3">Ponte a Prueba</h3>
                    <ul className="space-y-3">
                        {data.quizQuestions.map((item, index) => (
                           <details key={index} className="p-3 bg-slate-900/50 rounded-md cursor-pointer group">
                               <summary className="font-semibold text-white list-none flex justify-between items-center gap-2">
                                   <span className="flex-1">{item.question}</span>
                                   <span className="text-blue-400 text-xs uppercase font-bold mr-2 bg-blue-900/50 px-2 py-1 rounded-full">{item.type}</span>
                                   <svg className="w-4 h-4 transition-transform transform group-open:rotate-90" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" /></svg>
                               </summary>
                               <div className="text-slate-300 mt-2 pt-2 border-t border-slate-700 whitespace-pre-wrap"><strong>Respuesta:</strong> {item.answer}</div>
                           </details>
                        ))}
                    </ul>
                </div>
        ...
        <div className="flex flex-col h-full">
            <div className="flex-grow overflow-y-auto pr-2 space-y-4">
                {history.length === 0 ? (
                    <div className="flex items-center justify-center h-full text-slate-400 text-center">
                        <div>
                            <p className="mb-2">Soy tu IA para</p>
                            <strong className="text-blue-400 block">{subject}</strong>
                    ...
                    history.map((msg, index) => (
                        <div key={index} className={`flex items-start gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
                             {msg.role === 'model' && <div className="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center flex-shrink-0 mt-1"><BrainCircuitIcon className="w-5 h-5 text-white"/></div>}
                            <div className={`max-w-xl p-3 rounded-lg ${msg.role === 'user' ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-200'}`}>
                ...
                {isLoading && history[history.length-1]?.role === 'user' && (
                    <div className="flex items-start gap-3">
                        <div className="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center flex-shrink-0 mt-1"><BrainCircuitIcon className="w-5 h-5 text-white"/></div>
                        <div className="max-w-md p-3 rounded-lg bg-slate-800 text-slate-200">
            ...
            <form onSubmit={onSubmit} className="mt-4 flex gap-2">
                <input
                    ...
                    className="flex-grow bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                    disabled={isLoading}
                />
